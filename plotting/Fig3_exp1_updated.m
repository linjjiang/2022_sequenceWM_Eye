...
tbl = readtable(fullfile(output,'p_sd.csv'));
% temp = tbl{:,1:16};
...
%% Swap rate
nexttile
tbl = readtable(fullfile(output,'p_beta.csv'));
% temp = tbl{:,1:16};
% data = [temp(:,1:2:16);temp(:,2:2:16)];
data = tbl{:,1:8};
...
tbl = readtable(fullfile(output,'p_alpha.csv'));
% temp = tbl{:,1:16};
% data = [temp(:,1:2:16);temp(:,2:2:16)];
data = tbl{:,1:8};
...
%% Guessing rate
nexttile
tbl = readtable(fullfile(output,'p_gamma.csv'));
% temp = tbl{:,1:16};
% data = [temp(:,1:2:16);temp(:,2:2:16)];
data = tbl{:,1:8};
...
for ss = 1:size(p_ord_err,1) % for each participant
    for pp = 1:size(p_ord_err,3) % for each serial position
...
% hold on
% end
errorbar([1:4],mean(p_sp_perc(:,:,pp),1),std(p_sp_perc(:,:,pp),0,1)/sqrt(length(final_sind1)),'color',[0.5 0.5 0.5],'LineWidth',linewz,'LineStyle','--','Marker','s','MarkerSize',markersz)
hold on
errorbar([1:4],mean(p_sp_perc(:,:,pp+4),1),std(p_sp_perc(:,:,pp+4),0,1)/sqrt(length(final_sind2)),'color',[0 0 0],'LineWidth',linewz,'LineStyle','-','Marker','o','MarkerSize',markersz)
hold on
end
box off
set(gca,'FontSize',fz,'XTick',0:5,'XTickLabel',{'','1','2','3','4',''})
...
nexttile
tbl = readtable(fullfile(output,'p_orderr.csv'));
data = tbl{:,1:8};
...
print(f,'Fig3_exp1_updated.png','-dpng','-r300')

%saveas(f,fullfile(output,['Fig3_exp1.jpg']))
%print(f, '-depsc2', '-r300', '-tiff', '-loose', fullfile(output,['Fig3_exp1.eps']))
...
% 
% m = mean(data,'omitnan');
% sd = std(data,0,101,'omitnan')/sqrt(size(data,1));
% 
% % effects of serial position and cue
% % the figure has 8 columns, 1-4 serial positions for quadrant and order cue
% % respectively
