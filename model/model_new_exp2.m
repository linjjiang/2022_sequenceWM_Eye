...
% output = 'C:\Users\lj104\OneDrive\Documents\Linjing_Research\thesis\eye_data\exp2';

% add scripts to the path
addpath(genpath('./scripts/Cloned-Repo/'))

% load stats 
load(fullfile(output,'stats_exp2_quad.mat')) % stats1
load(fullfile(output,'stats_exp2_ord.mat')) % stats2

% %% model the v3_short, long delay condition - by cue
...
% idx = find(stats1.cue(ss,[1:64]+64*(cc-1)) == cc-1 & stats1.kept_trial(ss,[1:64]+64*(cc-1)) == 1);
% 
% % what we need:
...
% %stats1.model.mux(ss,cc) = fit(4);
% %stats1.model.muy(ss,cc) = fit(5);
% %stats1.model.mu(ss,cc) = sqrt(fit(4)^2 + fit(5)^2);
% stats1.model_nosys.sd(ss,cc) = fit(3); % standard deviation
% stats1.model_nosys.alpha(ss,cc) = 1-fit(1)-fit(2);
% stats1.model_nosys.beta(ss,cc) = fit(2);
% stats1.model_nosys.gamma(ss,cc) = fit(1);
% stats1.model_nosys.llh(ss,cc) = llh;
%     end
% end
% 
% %% Firs model the v3_short_delay, short delay condition - by cue
% for ss = 1:length(stats2.subs_id) % for each participant
%     % Let's model quadrant and order cue differently
%     for cc = 1:2 % 1, quadrant cue (0); 2, order cue (1)
% 
% % row (trial) index of the specific type of cue
% idx = find(stats2.cue(ss,[1:64]+64*(cc-1)) == cc-1 & stats2.kept_trial(ss,[1:64]+64*(cc-1)) == 1);
% 
% % what we need:
% % X and Y of primary saccade endpoints
% % X and Y of targets
...
% % stats.model.llh = llh;
% % stats.model.MemModel = model;
% %stats2.model.mux(ss,cc) = fit(4);
% %stats2.model.muy(ss,cc) = fit(5);
% %stats2.model.mu(ss,cc) = sqrt(fit(4)^2 + fit(5)^2);
% stats2.model_nosys.sd(ss,cc) = fit(3); % standard deviation
% stats2.model_nosys.alpha(ss,cc) = 1-fit(1)-fit(2);
% stats2.model_nosys.beta(ss,cc) = fit(2);
% stats2.model_nosys.gamma(ss,cc) = fit(1);
% stats2.model_nosys.llh(ss,cc) = llh;
%     end
% end

%% Standard model: quadrant cue - serial position by cue
...
%% Standard mixture model: order cue - serial position by cue
% only guessing and memory imprecision
...
%% Swap model: quadrant cue - serial position by cue
...
%% Swap model: order cue - serial position by cue
% only guessing, swapping, and memory imprecision
