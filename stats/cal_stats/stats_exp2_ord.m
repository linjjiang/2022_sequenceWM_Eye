% Statistical analysis for experiment 2, quadrant cue
...
% data directory
data_dir1 = './eye_data/v3_model_ord/';
file_dirs1 = [dir(fullfile(data_dir1,'sMGS*','s*'))];
...
run_names = str2double(cellfun(@(x) erase(x,[data_dir1,'sMGS']),{file_dirs1(ind).folder},'UniformOutput',false));
            ...
            p_resp = nan(16*length(runs_id1),1); % response type of that trial
            p_ord = nan(16*length(runs_id1),1); % serial position of the target
            p_quad = nan(16*length(runs_id1),1); % quadrant of the target
            p_img = nan(16*length(runs_id1),1); % image type of the target
            p_task = nan(16*length(runs_id1),1); % task type (cue type)
            p_ecc = nan(16*length(runs_id1),1); % eccentricity of the target
            p_xtar = nan(16*length(runs_id1),1); p_ytar = nan(16*length(runs_id1),1); % x and y location of the target in dva
            p_xtar_pix = nan(16*length(runs_id1),1); p_ytar_pix = nan(16*length(runs_id1),1); % x and y location of the target in pixel
            p_xntar_pix = nan(16*length(runs_id1),3); p_yntar_pix = nan(16*length(runs_id1),3); % x and y location of the three non-targets in pixel
            p_rm = nan(16*length(runs_id1),1); % trials that DO NOT need to be removed
            

            p_err = nan(16*length(runs_id1),1); % primary saccade error (that enters AOI of any stimulus)
            a_err = nan(16*length(runs_id1),1); % closest saccade error
            pt_err = nan(16*length(runs_id1),1); % primary saccade error (that enters target AOI)
            p_rt = nan(16*length(runs_id1),1); % primary saccade latency (that enters AOI of any stimulus)
            pt_rt = nan(16*length(runs_id1),1); % primary saccade latency (that enters target AOI)
            a_rt = nan(16*length(runs_id1),1); % closest saccade latency
            
            p_xend = nan(16*length(runs_id1),1); % primary saccade endpoint x location in dva
            p_yend = nan(16*length(runs_id1),1); % primary saccade endpoint y location in dva
            p_xend_pix = nan(16*length(runs_id1),1); p_yend_pix = nan(16*length(runs_id1),1); % primary saccade endpoint locations in pixel
            % primary saccade angular error, quadrant error, and serial
            % position (transposition) error (saccade minus target)
            p_theta_err = nan(16*length(runs_id1),1); p_quad_err = nan(16*length(runs_id1),1); 
            p_ord_err = nan(16*length(runs_id1),1); 
            % the most accurate saccade angular error, quadrant error, and transposition error (saccade minus target) 
            a_theta_err = nan(16*length(runs_id1),1); 
            a_quad_err = nan(16*length(runs_id1),1); 
            a_ord_err = nan(16*length(runs_id1),1);
...
%         end
        ...
        file = dir(fullfile(file_dirs1(ind(jj)).folder,file_dirs1(ind(jj)).name,'result','s*step6.mat'));
        %   fprintf('\n');
...
% across runs
stats2.resp_type(ii,:) = p_resp; % response type
stats2.p_err(ii,:) = p_err; % participants x trial
stats2.pt_err(ii,:) = pt_err;
stats2.a_err(ii,:) = a_err;
stats2.a_rt(ii,:) = a_rt;
stats2.p_rt(ii,:) = p_rt;
stats2.pt_rt(ii,:) = pt_rt;
stats2.ord(ii,:) = p_ord;
stats2.quad(ii,:) = p_quad;
stats2.kept_trial(ii,:) = p_rm;
stats2.img(ii,:) = p_img;
stats2.cue(ii,:) = p_task;
stats2.ecc(ii,:) = p_ecc;
stats2.p_xend_pix(ii,:) = p_xend_pix;
stats2.p_yend_pix(ii,:) = p_yend_pix;
% stats2.p_xend_deg(ii,:) = p_xend;
% stats2.p_yend_deg(ii,:) = p_yend;
stats2.xtar_pix(ii,:) = p_xtar_pix;
stats2.ytar_pix(ii,:) = p_ytar_pix;
% stats2.xtar_deg(ii,:) = p_xtar;
% stats2.ytar_deg(ii,:) = p_ytar;
stats2.xntar_pix(ii,:,:) = p_xntar_pix; % participants x trial x number of non-targets 
stats2.yntar_pix(ii,:,:) = p_yntar_pix;

%%%%%%%%%%%% Organized by Serial Position %%%%%%%%%%%%%%%%%%%%%%%%%

% preallocate
for kk = 1:4 % for each ordinal position
    for hh = 1:2 % for each task (cue type) - there is only one cue type for each participant
...
p_err_new(:,kk+(hh-1)*4) = [p_err(ind);nan(64-length(ind),1)]; % at most 64 trials per serial position

% most accurate saccade error to target
a_err_new(:,kk+(hh-1)*4) = [a_err(ind);nan(64-length(ind),1)];
%n_sac_new(:,kk+(hh-1)*4) = [n_sac(ind);nan(16-length(ind),1)];

% closest saccade latency
a_rt_new(:,kk+(hh-1)*4) = [a_rt(ind);nan(64-length(ind),1)];

% primary saccade for any stimulus, latency
p_rt_new(:,kk+(hh-1)*4) = [p_rt(ind);nan(64-length(ind),1)];
%n_tr(1,kk+(hh-1)*4) = sum(~isnan(p_err(ind)));

% primary saccade for target, error to target and latency
pt_err_new(:,kk+(hh-1)*4) = [pt_err(ind);nan(64-length(ind),1)];
pt_rt_new(:,kk+(hh-1)*4) = [pt_rt(ind);nan(64-length(ind),1)];

% primary saccade endpoint x and y location in dva and pixel
p_xend_new(:,kk+(hh-1)*4) = [p_xend(ind);nan(64-length(ind),1)];
p_yend_new(:,kk+(hh-1)*4) = [p_yend(ind);nan(64-length(ind),1)];
p_xend_pix_new(:,kk+(hh-1)*4) = [p_xend_pix(ind);nan(64-length(ind),1)];
p_yend_pix_new(:,kk+(hh-1)*4) = [p_yend_pix(ind);nan(64-length(ind),1)];

% target x and y location in dva and pixel
p_xtar_new(:,kk+(hh-1)*4) = [p_xtar(ind);nan(64-length(ind),1)];
p_ytar_new(:,kk+(hh-1)*4) = [p_ytar(ind);nan(64-length(ind),1)];
p_xtar_pix_new(:,kk+(hh-1)*4) = [p_xtar_pix(ind);nan(64-length(ind),1)];
p_ytar_pix_new(:,kk+(hh-1)*4) = [p_ytar_pix(ind);nan(64-length(ind),1)];

% primary saccade (for any stimulus) angular, quadrant, and serial position error
p_theta_err_new(:,kk+(hh-1)*4) = [p_theta_err(ind);nan(64-length(ind),1)];
p_quad_err_new(:,kk+(hh-1)*4) = [p_quad_err(ind);nan(64-length(ind),1)];
p_ord_err_new(:,kk+(hh-1)*4) = [p_ord_err(ind);nan(64-length(ind),1)];

% most accurate saccade angular, quadrant, and serial position error
a_theta_err_new(:,kk+(hh-1)*4) = [a_theta_err(ind);nan(64-length(ind),1)];
a_quad_err_new(:,kk+(hh-1)*4) = [a_quad_err(ind);nan(64-length(ind),1)];
a_ord_err_new(:,kk+(hh-1)*4) = [a_ord_err(ind);nan(64-length(ind),1)];
...
p_err_secc(:,kk+(hh-1)*4) = [p_err(ind1);nan(32-length(ind1),1)]; % at most 32 trials per serial position per eccentricity per cue type
p_err_lecc(:,kk+(hh-1)*4) = [p_err(ind2);nan(32-length(ind2),1)];
p_rt_secc(:,kk+(hh-1)*4) = [p_rt(ind1);nan(32-length(ind1),1)];
p_rt_lecc(:,kk+(hh-1)*4) = [p_rt(ind2);nan(32-length(ind2),1)];

% primary saccade (for target), error and latency, small and large eccentricity
pt_err_secc(:,kk+(hh-1)*4) = [pt_err(ind1);nan(32-length(ind1),1)];
pt_err_lecc(:,kk+(hh-1)*4) = [pt_err(ind2);nan(32-length(ind2),1)];
pt_rt_secc(:,kk+(hh-1)*4) = [pt_rt(ind1);nan(32-length(ind1),1)];
pt_rt_lecc(:,kk+(hh-1)*4) = [pt_rt(ind2);nan(32-length(ind2),1)];

% most accurate saccade, error, small and large eccentricity
a_err_secc(:,kk+(hh-1)*4) = [a_err(ind1);nan(32-length(ind1),1)];
a_err_lecc(:,kk+(hh-1)*4) = [a_err(ind2);nan(32-length(ind2),1)];
a_rt_secc(:,kk+(hh-1)*4) = [a_rt(ind1);nan(32-length(ind1),1)];
a_rt_lecc(:,kk+(hh-1)*4) = [a_rt(ind2);nan(32-length(ind2),1)];

% primary saccade (for any stimulus), endpoint x and y location in pixel, small and
% large eccentricity
p_xend_pix_new_secc(:,kk+(hh-1)*4) = [p_xend_pix(ind1);nan(32-length(ind1),1)];
p_yend_pix_new_secc(:,kk+(hh-1)*4) = [p_yend_pix(ind1);nan(32-length(ind1),1)];
p_xend_pix_new_lecc(:,kk+(hh-1)*4) = [p_xend_pix(ind2);nan(32-length(ind2),1)];
p_yend_pix_new_lecc(:,kk+(hh-1)*4) = [p_yend_pix(ind2);nan(32-length(ind2),1)];

% target x and y location in pixel, small and large eccentricity
p_xtar_pix_new_secc(:,kk+(hh-1)*4) = [p_xtar_pix(ind1);nan(32-length(ind1),1)];
p_ytar_pix_new_secc(:,kk+(hh-1)*4) = [p_ytar_pix(ind1);nan(32-length(ind1),1)];
p_xtar_pix_new_lecc(:,kk+(hh-1)*4) = [p_xtar_pix(ind2);nan(32-length(ind2),1)];
p_ytar_pix_new_lecc(:,kk+(hh-1)*4) = [p_ytar_pix(ind2);nan(32-length(ind2),1)];
    end
end
...
stats2.p_err_ord(ii,:,:) = p_err_new; 
stats2.a_err_ord(ii,:,:) = a_err_new;
stats2.a_rt_ord(ii,:,:) = a_rt_new;
%stats2.n_sac_ord(ii,:,:) = n_sac_new;
stats2.p_rt_ord(ii,:,:) = p_rt_new;
stats2.pt_err_ord(ii,:,:) = pt_err_new;
stats2.pt_rt_ord(ii,:,:) = pt_rt_new;
%stats2.n_tr_ord(ii,:,:) = n_tr;

stats2.p_xend_ord(ii,:,:) = p_xend_new;
stats2.p_yend_ord(ii,:,:) = p_yend_new;
stats2.p_xtar_ord(ii,:,:) = p_xtar_new;
stats2.p_ytar_ord(ii,:,:) = p_ytar_new;

stats2.p_xend_pix_ord(ii,:,:) = p_xend_pix_new;
stats2.p_yend_pix_ord(ii,:,:) = p_yend_pix_new;
stats2.p_xtar_pix_ord(ii,:,:) = p_xtar_pix_new;
stats2.p_ytar_pix_ord(ii,:,:) = p_ytar_pix_new;

stats2.p_xend_pix_ord_secc(ii,:,:) = p_xend_pix_new_secc;
stats2.p_xend_pix_ord_lecc(ii,:,:) = p_xend_pix_new_lecc;
stats2.p_yend_pix_ord_secc(ii,:,:) = p_yend_pix_new_secc;
stats2.p_yend_pix_ord_lecc(ii,:,:) = p_yend_pix_new_lecc;
stats2.p_xtar_pix_ord_secc(ii,:,:) = p_xtar_pix_new_secc;
stats2.p_xtar_pix_ord_lecc(ii,:,:) = p_xtar_pix_new_lecc;
stats2.p_ytar_pix_ord_secc(ii,:,:) = p_ytar_pix_new_secc;
stats2.p_ytar_pix_ord_lecc(ii,:,:) = p_ytar_pix_new_lecc;

stats2.p_err_ord_secc(ii,:,:) = p_err_secc;
stats2.p_err_ord_lecc(ii,:,:) = p_err_lecc;
stats2.p_rt_ord_secc(ii,:,:) = p_rt_secc;
stats2.p_rt_ord_lecc(ii,:,:) = p_rt_lecc;

stats2.pt_err_ord_secc(ii,:,:) = pt_err_secc;
stats2.pt_err_ord_lecc(ii,:,:) = pt_err_lecc;
stats2.pt_rt_ord_secc(ii,:,:) = pt_rt_secc;
stats2.pt_rt_ord_lecc(ii,:,:) = pt_rt_lecc;

stats2.a_err_ord_secc(ii,:,:) = a_err_secc;
stats2.a_err_ord_lecc(ii,:,:) = a_err_lecc;
stats2.a_rt_ord_secc(ii,:,:) = a_rt_secc;
stats2.a_rt_ord_lecc(ii,:,:) = a_rt_lecc;

% calculate systematic and unsystematic error for primary saccade (for any
% stimulus)
[stats2.p_sys_ord(ii,:),stats2.p_unsys_ord(ii,:)] = cal_sys_unsys(p_xend_pix_new,p_yend_pix_new,p_xtar_pix_new,p_ytar_pix_new,edf);
[stats2.p_sys_ord_secc(ii,:),stats2.p_unsys_ord_secc(ii,:)] = cal_sys_unsys(...
    p_xend_pix_new_secc,p_yend_pix_new_secc,p_xtar_pix_new_secc,p_ytar_pix_new_secc,edf);
[stats2.p_sys_ord_lecc(ii,:),stats2.p_unsys_ord_lecc(ii,:)] = cal_sys_unsys(...
    p_xend_pix_new_lecc,p_yend_pix_new_lecc,p_xtar_pix_new_lecc,p_ytar_pix_new_lecc,edf);
 
stats2.p_theta_err_ord(ii,:,:) = p_theta_err_new;
stats2.p_quad_err_ord(ii,:,:) = p_quad_err_new;
stats2.p_ord_err_ord(ii,:,:) = p_ord_err_new;

stats2.a_theta_err_ord(ii,:,:) = p_theta_err_new;
stats2.a_quad_err_ord(ii,:,:) = p_quad_err_new;
stats2.a_ord_err_ord(ii,:,:) = p_ord_err_new;
...
for kk = 1:4 % for each quadrant
    ...
    % primary saccade for any stimulus, error to target, N participant x 8 conditions
    % (serial order 1-4 for quadarnt cue, 1-4 for order cue)
p_err_new(:,kk+(hh-1)*4) = [p_err(ind);nan(64-length(ind),1)]; % at most 16 trials per serial position
...
% Number of participant x Number of trials x Number of experimental
% conditions
% Here we have 32 conditions,
% serial order 1-4 for quadrant cue, and 1-4 for order cue
stats2.p_err_quad(ii,:,:) = p_err_new; 
stats2.a_err_quad(ii,:,:) = a_err_new;
stats2.a_rt_quad(ii,:,:) = a_rt_new;
%stats2.n_sac_quad(ii,:,:) = n_sac_new;
stats2.p_rt_quad(ii,:,:) = p_rt_new;
stats2.pt_err_quad(ii,:,:) = pt_err_new;
stats2.pt_rt_quad(ii,:,:) = pt_rt_new;
%stats2.n_tr_quad(ii,:,:) = n_tr;

stats2.p_xend_quad(ii,:,:) = p_xend_new;
stats2.p_yend_quad(ii,:,:) = p_yend_new;
stats2.p_xtar_quad(ii,:,:) = p_xtar_new;
stats2.p_ytar_quad(ii,:,:) = p_ytar_new;

stats2.p_xend_pix_quad(ii,:,:) = p_xend_pix_new;
stats2.p_yend_pix_quad(ii,:,:) = p_yend_pix_new;
stats2.p_xtar_pix_quad(ii,:,:) = p_xtar_pix_new;
stats2.p_ytar_pix_quad(ii,:,:) = p_ytar_pix_new;

stats2.p_xend_pix_quad_secc(ii,:,:) = p_xend_pix_new_secc;
stats2.p_xend_pix_quad_lecc(ii,:,:) = p_xend_pix_new_lecc;
stats2.p_yend_pix_quad_secc(ii,:,:) = p_yend_pix_new_secc;
stats2.p_yend_pix_quad_lecc(ii,:,:) = p_yend_pix_new_lecc;
stats2.p_xtar_pix_quad_secc(ii,:,:) = p_xtar_pix_new_secc;
stats2.p_xtar_pix_quad_lecc(ii,:,:) = p_xtar_pix_new_lecc;
stats2.p_ytar_pix_quad_secc(ii,:,:) = p_ytar_pix_new_secc;
stats2.p_ytar_pix_quad_lecc(ii,:,:) = p_ytar_pix_new_lecc;

stats2.p_err_quad_secc(ii,:,:) = p_err_secc;
stats2.p_err_quad_lecc(ii,:,:) = p_err_lecc;
stats2.p_rt_quad_secc(ii,:,:) = p_rt_secc;
stats2.p_rt_quad_lecc(ii,:,:) = p_rt_lecc;

stats2.pt_err_quad_secc(ii,:,:) = pt_err_secc;
stats2.pt_err_quad_lecc(ii,:,:) = pt_err_lecc;
stats2.pt_rt_quad_secc(ii,:,:) = pt_rt_secc;
stats2.pt_rt_quad_lecc(ii,:,:) = pt_rt_lecc;

stats2.a_err_quad_secc(ii,:,:) = a_err_secc;
stats2.a_err_quad_lecc(ii,:,:) = a_err_lecc;
stats2.a_rt_quad_secc(ii,:,:) = a_rt_secc;
stats2.a_rt_quad_lecc(ii,:,:) = a_rt_lecc;

% calculate systematic and unsystematic error for primary saccade (for any
% stimulus)
[stats2.p_sys_quad(ii,:),stats2.p_unsys_quad(ii,:)] = cal_sys_unsys(p_xend_pix_new,p_yend_pix_new,p_xtar_pix_new,p_ytar_pix_new,edf);
[stats2.p_sys_quad_secc(ii,:),stats2.p_unsys_quad_secc(ii,:)] = cal_sys_unsys(...
    p_xend_pix_new_secc,p_yend_pix_new_secc,p_xtar_pix_new_secc,p_ytar_pix_new_secc,edf);
[stats2.p_sys_quad_lecc(ii,:),stats2.p_unsys_quad_lecc(ii,:)] = cal_sys_unsys(...
    p_xend_pix_new_lecc,p_yend_pix_new_lecc,p_xtar_pix_new_lecc,p_ytar_pix_new_lecc,edf);
 
stats2.p_theta_err_quad(ii,:,:) = p_theta_err_new;
stats2.p_quad_err_quad(ii,:,:) = p_quad_err_new;
stats2.p_ord_err_quad(ii,:,:) = p_ord_err_new;

stats2.a_theta_err_quad(ii,:,:) = p_theta_err_new;
stats2.a_quad_err_quad(ii,:,:) = p_quad_err_new;
stats2.a_ord_err_quad(ii,:,:) = p_ord_err_new;
...
for kk = 1:4 % for each image type
    ...
    % primary saccade for any stimulus, error to target, N participant x 32 conditions
    % (serial order 1-4 for quadarnt cue, 1-4 for order cue)
p_err_new(:,kk+(hh-1)*4) = [p_err(ind);nan(64-length(ind),1)]; % at most 64 trials per serial position

% most accurate saccade error to target
a_err_new(:,kk+(hh-1)*4) = [a_err(ind);nan(64-length(ind),1)];
%n_sac_new(:,kk+(hh-1)*4) = [n_sac(ind);nan(64-length(ind),1)];
...
% Here we have 8 conditions,
% serial order 1-4 for quadrant cue, and 1-4 for order cue
stats2.p_err_img(ii,:,:) = p_err_new; 
stats2.a_err_img(ii,:,:) = a_err_new;
stats2.a_rt_img(ii,:,:) = a_rt_new;
%stats2.n_sac_img(ii,:,:) = n_sac_new;
stats2.p_rt_img(ii,:,:) = p_rt_new;
stats2.pt_err_img(ii,:,:) = pt_err_new;
stats2.pt_rt_img(ii,:,:) = pt_rt_new;
%stats2.n_tr_img(ii,:,:) = n_tr;

stats2.p_xend_img(ii,:,:) = p_xend_new;
stats2.p_yend_img(ii,:,:) = p_yend_new;
stats2.p_xtar_img(ii,:,:) = p_xtar_new;
stats2.p_ytar_img(ii,:,:) = p_ytar_new;

stats2.p_xend_pix_img(ii,:,:) = p_xend_pix_new;
stats2.p_yend_pix_img(ii,:,:) = p_yend_pix_new;
stats2.p_xtar_pix_img(ii,:,:) = p_xtar_pix_new;
stats2.p_ytar_pix_img(ii,:,:) = p_ytar_pix_new;

stats2.p_xend_pix_img_secc(ii,:,:) = p_xend_pix_new_secc;
stats2.p_xend_pix_img_lecc(ii,:,:) = p_xend_pix_new_lecc;
stats2.p_yend_pix_img_secc(ii,:,:) = p_yend_pix_new_secc;
stats2.p_yend_pix_img_lecc(ii,:,:) = p_yend_pix_new_lecc;
stats2.p_xtar_pix_img_secc(ii,:,:) = p_xtar_pix_new_secc;
stats2.p_xtar_pix_img_lecc(ii,:,:) = p_xtar_pix_new_lecc;
stats2.p_ytar_pix_img_secc(ii,:,:) = p_ytar_pix_new_secc;
stats2.p_ytar_pix_img_lecc(ii,:,:) = p_ytar_pix_new_lecc;

stats2.p_err_img_secc(ii,:,:) = p_err_secc;
stats2.p_err_img_lecc(ii,:,:) = p_err_lecc;
stats2.p_rt_img_secc(ii,:,:) = p_rt_secc;
stats2.p_rt_img_lecc(ii,:,:) = p_rt_lecc;

stats2.pt_err_img_secc(ii,:,:) = pt_err_secc;
stats2.pt_err_img_lecc(ii,:,:) = pt_err_lecc;
stats2.pt_rt_img_secc(ii,:,:) = pt_rt_secc;
stats2.pt_rt_img_lecc(ii,:,:) = pt_rt_lecc;

stats2.a_err_img_secc(ii,:,:) = a_err_secc;
stats2.a_err_img_lecc(ii,:,:) = a_err_lecc;
stats2.a_rt_img_secc(ii,:,:) = a_rt_secc;
stats2.a_rt_img_lecc(ii,:,:) = a_rt_lecc;

% calculate systematic and unsystematic error for primary saccade (for any
% stimulus)
[stats2.p_sys_img(ii,:),stats2.p_unsys_img(ii,:)] = cal_sys_unsys(p_xend_pix_new,p_yend_pix_new,p_xtar_pix_new,p_ytar_pix_new,edf);
[stats2.p_sys_img_secc(ii,:),stats2.p_unsys_img_secc(ii,:)] = cal_sys_unsys(...
    p_xend_pix_new_secc,p_yend_pix_new_secc,p_xtar_pix_new_secc,p_ytar_pix_new_secc,edf);
[stats2.p_sys_img_lecc(ii,:),stats2.p_unsys_img_lecc(ii,:)] = cal_sys_unsys(...
    p_xend_pix_new_lecc,p_yend_pix_new_lecc,p_xtar_pix_new_lecc,p_ytar_pix_new_lecc,edf);
 
stats2.p_theta_err_img(ii,:,:) = p_theta_err_new;
stats2.p_quad_err_img(ii,:,:) = p_quad_err_new;
stats2.p_ord_err_img(ii,:,:) = p_ord_err_new;

stats2.a_theta_err_img(ii,:,:) = p_theta_err_new;
stats2.a_quad_err_img(ii,:,:) = p_quad_err_new;
stats2.a_ord_err_img(ii,:,:) = p_ord_err_new;
end

% store the subject id and run id
stats2.subs_id = subs_id1;
stats2.runs_id = {'serial position 1, quadrant cue',...
...
%% Save the stats
% stats2.final_sind = final_sind1;
save(fullfile(output,'stats_exp2_ord.mat'),'stats2')
